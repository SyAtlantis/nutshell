...
it('Navigation test: node not running', async () => {
  ...
  await page.waitForFunction(`document.title === 'Welcome to NUTSHELL'`, { timeout: 8000 })
}, ms.minutes(1))
...
it('Navigation test: node running', async () => {
  ...
  await page.goto(appUrl)
  await waitForTitle('Status - NUTSHELL')

  await page.click('nav a[href="#/files/"]')
  await waitForTitle('Files - NUTSHELL')

  await page.click('nav a[href="#/explore"]')
  await waitForTitle('Explore - IPLD')

  await page.click('nav a[href="#/peers"]')
  await waitForTitle('Peers - NUTSHELL')

  await page.click('nav a[href="#/settings"]')
  await waitForTitle('Settings - NUTSHELL')

  await page.click('nav a[href="#/"]')
  await waitForTitle('Status - NUTSHELL')
}, ms.minutes(1))

// Pretend to be NUTSHELL companion so we can mock out ipfs responses
