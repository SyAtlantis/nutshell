        ...
        <div className='flex-auto pr3 lh-copy charcoal'>
          <Box>
            ...
            <Trans i18nKey='configureApiPort.paragraph1'>
              <p>If your NUTSHELL node is configured with a <a className='link blue' href='https://github.com/ipfs/go-ipfs/blob/master/docs/config.md#addresses' target='_blank' rel='noopener noreferrer'>custom API address</a>, please set it here</p>
            </Trans>
            <ApiAddressForm
              t={t}
    ...
    return (
      <div>
        ...
        <Trans i18nKey='connected.paragraph2'>
          <p>You can always come back to this address to change the NUTSHELL node you're connected to.</p>
        </Trans>
      </div>
    )
  }
      ...
      { !sameOrigin && (
        <div>
          <Trans i18nKey='notConnected.paragraph2'>
            <p>Make sure you <a className='link blue' href='https://github.com/ipfs-shipyard/ipfs-webui#configure-ipfs-api-cors-headers'>configure your NUTSHELL API</a> to allow cross-origin (CORS) requests, running the commands below:</p>
          </Trans>
          <Shell>
            <code className='db'>$ nutshell config --json API.HTTPHeaders.Access-Control-Allow-Origin '[{addOrigin && `"${origin}", `}]'</code>
            <code className='db'>$ nutshell config --json API.HTTPHeaders.Access-Control-Allow-Methods '["PUT", "GET", "POST"]'</code>
          </Shell>
        </div>
      )}
      <Trans i18nKey='notConnected.paragraph3'>
        <p>Start an NUTSHELL daemon in a terminal:</p>
      </Trans>
      <Shell>
        <code className='db'>$ nutshell daemon</code>
        <code className='db'>Initializing daemon...</code>
        <code className='db'>API server listening on /ip4/127.0.0.1/tcp/5001</code>
      </Shell>
      <Trans i18nKey='notConnected.paragraph4'>
        <p>For more info on how to get started with NUTSHELL you can <a className='link blue' href='https://docs.ipfs.io/introduction/usage/'>read the guide</a>.</p>
      </Trans>
    </div>
  )
}
