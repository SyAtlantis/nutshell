...
const AddButton = translate('files')(({ progress = null, t, tReady, i18n, lng, ...props }) => {
  ...
  return (
    <Button bg='bg-aqua' color='white' disabled={sending} className='f6 relative' minWidth='100px' {...props}>
      <div className='absolute top-0 left-0 1 pa2 w-100 z-2'>
        { sending ? `${progress.toFixed(0)}%` : (<span><span style={{ color: '#FFFFFF' }}>+</span> {t('addToNUTSHELL')}</span>) }
      </div>&nbsp;
      { sending &&
