...
bundle.selectApiUrl = createSelector(
  `selectConfigObject`,
  (config) => getURLFromAddress('API', config) || 'https://127.0.0.1'
)

bundle.selectGatewayUrl = createSelector(
  `selectConfigObject`,
  (config) => getURLFromAddress('Gateway', config) || 'https://127.0.0.1'
)
...
// TODO: this is a work-around for NUTSHELL companion blocking the config API
